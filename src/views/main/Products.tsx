	...
	let Product = ({ item, index }) => {
		let initialFields: FieldProps[] = [
			...
			},
			{
				type: FieldType.LINE,
				size: FieldSize.FULL,
				title: strings.ndsInput,
				columns: [
					{
						type: FieldType.SELECT,
						title: strings.bet,
						size: FieldSize.QUARTER,
						placeholder: strings.bet,
						name: "VatRate",
						disabled: index !== editingIndex,
						staticValue: [
							{ label: "20%", value: 0, actualValue: 20 },
							{ label: "15%", value: 1, actualValue: 15 },
							{ label: "7%", value: 2, actualValue: 7 },
							{ label: "0%", value: 2, actualValue: 0 },
							{ label: "Без НДС", value: 3, actualValue: -1 }
						]
					},
					{
						type: FieldType.INPUT,
			...
			{
				type: FieldType.INPUT,
				title: strings.totalCostWithNDS,
				placeholder: strings.totalCostWithNDS,
				size: FieldSize.FULL,
				name: "DeliverySumWithVat",
				disabled: index !== editingIndex,
				validation: { float: true }
			}
		];
		...
		<View style={styles.container}>
			...
			<View style={styles.fixedButton}>
				<GradientButton
					...
					text={() => (
						<AntDesign name="plus" color={colors.white} size={26} />
					)}
					fontSize={30}
					style={{ padding: 15, paddingHorizontal: 15 }}
				/>
			</View>
		</View>
	);
